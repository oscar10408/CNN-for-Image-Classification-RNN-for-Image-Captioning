...
def conv_forward(x, w):
    ...
    ###########################################################################
    for x1 in range(H_prime):
        for y in range(W_prime):
            out[:,:, x1, y] = np.dot(x[:,:, x1:x1+HH, y:y+WW].reshape(N, -1),
                                     w.reshape(F, -1).T)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward(dout, cache):
    ...
    for x1 in range(HH):
        for y in range(WW):
            dw[:,:, x1, y] = np.dot(np.transpose(dout, (1,0,2,3)).reshape(F, -1),
                                    np.transpose(x[:,:, x1:x1+H_prime, y:y+W_prime], (1,0,2,3)).reshape(C, -1).T)
            
    w_flip = w_pad[:,:,::-1,::-1]
    _, _, newH, newW = w_flip.shape

    for x1 in range(H):
        for y in range(W):
            dx[:,:, x1, y] = np.dot(dout.reshape(N,-1),
                                    np.transpose(w_flip[:,:, newH-H_prime-x1:newH-x1, newW-W_prime-y:newW-y], (1,0,2,3)).reshape(C, -1).T)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw
