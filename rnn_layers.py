...
def temporal_fc_forward(x, w, b):
    ...
    N, T, D = x.shape
    _, M = w.shape
    x_reshaped = np.reshape(x, (N*T, D))
    product = np.dot(x_reshaped, w)
    out = np.reshape(product, (N, T, M)) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, out)
...
def temporal_fc_backward(dout, cache):
    ...
    dout_reshaped = np.reshape(dout, (N*T, M))
    dx = np.dot(dout_reshaped, w.T)
    dx = np.reshape(dx, (N, T, D))

    x_reshaped = np.reshape(x, (N*T, D))
    dw = np.dot(x_reshaped.T, dout_reshaped)
