...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        # Forward
        hiddenOut1, fcCache1 = fc_forward(features, W_proj, b_proj)

        cap_in, capInCache = word_embedding_forward(captions_in, W_embed)
        cap_in = np.transpose(cap_in, (1,0,2))

        hiddenState, rnnForwardCache = rnn_forward(cap_in, hiddenOut1, Wx, Wh, b)
        hiddenState = np.transpose(hiddenState, (1,0,2))

        vocabScore, tempFcCache = temporal_fc_forward(hiddenState, W_vocab, b_vocab)

        loss, dScore = temporal_softmax_loss(vocabScore, captions_out, mask)

        # Backward
        dHiddenState, grads['W_vocab'], grads['b_vocab'] = temporal_fc_backward(dScore, tempFcCache)
        dHiddenState = np.transpose(dHiddenState, (1,0,2))

        dWordEmb, dHiddeen1, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dHiddenState, rnnForwardCache)
        dWordEmb = np.transpose(dWordEmb, (1, 0, 2))

        grads['W_embed'] = word_embedding_backward(dWordEmb, capInCache)
        dfeatures, grads['W_proj'], grads['b_proj'] = fc_backward(dHiddeen1, fcCache1)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            ...
            vocabScore, _ = fc_forward(h, W_vocab, b_vocab)
            captions[:, t] = np.argmax(vocabScore, axis=1)
